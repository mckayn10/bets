...
export default class SearchableDropDown extends Component {
  ...
  searchedItems = searchedText => {
    ...
    if (!setSort && typeof setSort !== 'function') {
      setSort = (item, searchedText) => {
        ...
        return name.toLowerCase().indexOf(searchedText.toLowerCase()) > -1
      };
    }
        ...
        >
          {
            this.props.selectedItems && this.props.selectedItems.length > 0 && this.props.selectedItems.find(x => x.id === item.id)
              ?
              <Text style={{ ...this.props.itemTextStyle }}>{item.firstName} {item.lastName}</Text>
              :
  ...
  renderTextInput = () => {
    ...
    const oldSupport = [
      ...
      {
        key: 'value',
        val: this.state.item ? this.state.item.lastName : ''
      },
      {
        key: 'style',
    ...
    return (
      <Input
        {...textInputProps}
        containerStyle={{backgroundColor: Colors.grayLight, height: 50, borderRadius: 5, borderWidth: 1, borderColor: Colors.grayLight}}
        inputContainerStyle={{borderBottomWidth: 0}}
        leftIcon={<MaterialIcons name="search" size={20} color="black" />}
        onBlur={(e) => {
    ...
    return (
      ...
      >
        {this.state.useOpacityBackground && this.state.focus
          ? <View style={{ backgroundColor: 'white', width: 500, height: 1000, position: 'absolute', opacity: .9 }}></View>
          : null
        }
        ...
        {items.map((item, index) => {
          return (
            <View key={index} style={{
              width: (item.firstName.length + item.lastName.length * 8) + 60,
              ...
              <Text style={{ color: '#555' }}>{item.firstName} {item.lastName}</Text>
              <Pressable onPress={() => setTimeout(() => { this.props.onRemoveItem(item, index) }, 0)} style={{ backgroundColor: '#f16d6b', alignItems: 'center', justifyContent: 'center', width: 25, height: 25, borderRadius: 100, marginLeft: 10 }}>
                <Text>X</Text>
              </Pressable>
            </View>
